    ...
    <Link href={`/sharehouse/projects/${project.id}`} className="block">
      <div className={`group relative bg-surface-container-lowest p-4 pb-12 shadow-polaroid rounded-sm transform transition-all duration-500 hover:rotate-0 hover:scale-105 ${rotate} ${className}`}>
        ...
        <div className="relative aspect-square overflow-hidden mb-6 bg-surface-container-low border-8 border-white shadow-inner">
          ...
          <div className="absolute top-4 right-4 bg-secondary-container text-on-secondary-container px-3 py-1.5 rounded-sm font-hand text-sm font-bold shadow-md -rotate-6 border-b-2 border-r-2 border-black/5 z-10">
            {project.status}
          </div>
        </div>
        ...
        {/* Content */}
        <div className="px-2">
          <h3 className="text-xl font-headline font-black text-on-surface leading-tight mb-3 group-hover:text-primary transition-colors duration-300">
            {project.title}
          </h3>
          <p className="text-on-surface-variant text-sm font-body leading-relaxed line-clamp-3">
            {project.description}
          </p>
        </div>
