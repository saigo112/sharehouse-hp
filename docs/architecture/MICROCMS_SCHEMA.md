...
各プログラム詳細ページの選択肢をmicroCMSから編集する場合は、`programs` APIに次のフィールドを追加します。

| フィールドID | 種別 | 用途 |
| --- | --- | --- |
| `choiceHeading` | テキスト | 選択欄の見出し（例：やってみたいことを選ぶ） |
| `choiceIntroduction` | テキストエリア | 選択方法や注意事項 |
| `choiceOptions` | 繰り返しフィールド | プログラムごとの選択肢一覧 |

`choiceOptions`で使うカスタムフィールドを作成し、次の2項目を設定します。

| フィールドID | 種別 | 用途 |
| --- | --- | --- |
| `title` | テキスト | 選択肢名（例：田んぼのしごと） |
| `description` | テキストエリア | カードに表示する短い説明 |

`choiceOptions`が空の場合は、現在コードに登録されている初期選択肢が表示されます。1件以上入力すると、そのプログラムではmicroCMSの選択肢へすべて置き換わります。表示順はmicroCMS上の並び順と同じです。
...
- スタッフは `/schedule/manage` から予定を登録・編集・削除できる。種類を選ぶと公開用タグが自動で付く（検索エンジンには掲載しない設定）。
